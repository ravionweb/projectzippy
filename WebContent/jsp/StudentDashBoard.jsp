...
function alertDialog (prompt) {
	...
	$("#idAlertDialog").modal ("show");
     ...
     jQuery(document).ready(function () {
			...
			//assignmentLoad();
			setTimeout(assignmentLoad, 400);
			utilitiesLoad();
			//academicsLoad();
							
			setTimeout(academicsLoad, 400);				
		});
	...
	function sendQuickMSG(){
		...
		var ipaddr = '<%=request.getRemoteAddr()%>';	
			if (!$('input[name="quickCheck"]').is(':checked')) {
				setError1("THCheck","Please select checkbox","QuickError");
				jQuery("#THCheck").focus();
			    return false;
			...
			if (msgRes == "Success") {
				setError1("THCheck","Success","QuickError");
				jQuery("#THCheck").focus();
				$('input[name=quickCheck]').attr('checked', false);
				jQuery("#QuickMsg").val("");
				
			} else {
				setError1("THCheck","Failed","QuickError");
				jQuery("#THCheck").focus();
			}			
			});
	}
  ...
  <legend><img src="img/list_add_user.PNG" class="img-circle">&nbsp;&nbsp;Student Dashboard</legend>
